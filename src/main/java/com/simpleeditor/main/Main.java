...
public class Main {
	...
	public void createNewProject() {
		...
		int answer = JOptionPane.showConfirmDialog(null, "Start a new project? \nThe unsaved map will be lost.\n ",
				"New Project", 0);
		if (answer == 0) {
			resetMapTileNum();
	...
	private void importTile() {
		...
		filess.sort((f1, f2) -> f1.getName().compareTo(f2.getName()));
	...
	public void scaleImage() {
		for (int i = 0; i < this.tiles.size(); i++) {
			Image image48 = ((Tile) this.tiles.get(i)).image.getScaledInstance((int) (48.0D * this.gui.scale),
					(int) (48.0D * this.gui.scale), 1);
	...
	private void saveTileData() {
		try {
			BufferedWriter bw = new BufferedWriter(new FileWriter(
					String.valueOf(this.tileDataFileDirectory) + this.tileDataFileName, StandardCharsets.UTF_8));
			for (int i = 0; i < this.tiles.size(); i++) {
				String line = "".concat("" + i).concat(";").concat(this.tiles.get(i).name).concat(";")
						.concat("" + this.tiles.get(i).collision);
				bw.write(line);
		...
		try {
			...
			br.lines().forEach(l -> {
				...
				if (lineSplit.length == 3) {
					...
					Integer index = Integer.valueOf(lineSplit[0]);
					if (index < this.tiles.size()) {
						Tile tile = this.tiles.get(index);
						if (lineSplit[1].equals(tile.name)) {
							tile.collision = Boolean.valueOf(lineSplit[2]);
						}
					}
				}
	...
	private void savingMap() {
		try {
			BufferedWriter bw = new BufferedWriter(
					new FileWriter(String.valueOf(this.mapFileDirectory) + this.mapName));
			int col = 0;
			int row = 0;
			while (col < this.maxWorldCol && row < this.maxWorldRow) {
				String numberAsString = String.valueOf(this.mapTileNum[col][row]);
				numberAsString = numberAsString.length() == 1 ? "0".concat(numberAsString) : numberAsString;
	...
	public void loadMap() {
		...
		try {
			BufferedReader br0 = new BufferedReader(
					new FileReader(String.valueOf(this.mapFileDirectory) + this.mapName));
	...
	public void showMapSizeNotification(int size) {
		...
		int result = JOptionPane.showConfirmDialog(null,
				"Changing the map size will reset the current map. \nAre you sure to change it?", "Set Map Size\n ", 0,
				3);
		if (result == 0)
			setMapSize(size);
	}
	...
	public void refreshEditorImage() {
		...
		BufferedImage editorImage = new BufferedImage(this.gui.defaultScreenCol * this.gui.tileSize,
				this.gui.defaultScreenRow * this.gui.tileSize, 2);
	...
	public void drawOnWorldMap(int worldCol, int worldRow) {
		...
		if (this.tiles.size() > 0) {
			this.wg2.drawImage(((Tile) this.tiles.get(tileNum)).image, worldCol * 16, worldRow * 16,
					(ImageObserver) null);
			this.gui.worldMapPanel.drawGra(this.worldImage, 0, 0, this.gui.worldMPsize, this.gui.worldMPsize);
			this.gui.window.requestFocus();
		}
